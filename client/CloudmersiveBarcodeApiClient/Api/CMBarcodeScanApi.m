#import "CMBarcodeScanApi.h"
#import "CMQueryParamCollection.h"
#import "CMApiClient.h"
#import "CMBarcodeScanResult.h"


@interface CMBarcodeScanApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation CMBarcodeScanApi

NSString* kCMBarcodeScanApiErrorDomain = @"CMBarcodeScanApiErrorDomain";
NSInteger kCMBarcodeScanApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) barcodeScanImageWithImageFile: (NSURL*) imageFile
    ...
    if (imageFile == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kCMBarcodeScanApiErrorDomain code:kCMBarcodeScanApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/barcode/scan/image"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"CMBarcodeScanResult*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((CMBarcodeScanResult*)data, error);
                                }
                            }];
}
